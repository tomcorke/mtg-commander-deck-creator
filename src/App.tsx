...
type CommanderCard = ScryfallCard & {
...
const cardTags = (card: ScryfallCard, category = '') =>
  tagsFor(`${card.type_line}\n${cardText(card)}\n${category}`, card.type_line)
const toCard = (card: ScryfallCard, reason: string, category = ''): Card => ({
  ...toRecommendationCard(card, reason, category),
  source: 'scryfall',
})
const edhrecSlug = (url: string | undefined, name: string) =>
...
const selectableThemes = Object.keys(themeCommanders).filter((name) =>
...
const themeSearchTerms: Record<string, string> = {
  Tokens: 'o:token',
...
const scryfallImage = (card: ScryfallCard) =>
...
const cardScryfallUri = (card: Pick<Card, 'scryfallUri' | 'set' | 'collectorNumber'>) =>
  card.scryfallUri ??
  `https://scryfall.com/card/${card.set}/${encodeURIComponent(card.collectorNumber)}`
const cardTypeLine = (card: Pick<DeckCard, 'typeLine' | 'power' | 'toughness'>) =>
...
const cardPrintingsUri = (card: Pick<Card, 'name'>) =>
...
const defaultFinish = (finishes?: CardFinish[]) =>
  finishes?.includes('nonfoil') ? 'nonfoil' : finishes?.[0]
const commanderPrintingOptions = (cards: CommanderCard[]) =>
  cards
    .flatMap((printing) => {
...
const cardPrintingOptions = (cards: ScryfallCard[]) =>
  cards
    .flatMap((printing) => {
      const image = printing.image_uris?.normal ?? printing.card_faces?.[0]?.image_uris?.normal
      return image
...
function App() {
  ...
  const [commanderDetails, setCommanderDetails] = useState<CommanderDetails | null>(
    savedDeckState?.commanderDetails ?? null,
  )
  ...
  const [recommendationStyle, setRecommendationStyle] = useStoredOption<RecommendationStyle>(
    'recommendationStyle',
    () => savedDeckState?.recommendationStyle ?? 'balanced',
  )
  const [collectionSets, setCollectionSets] = useState<string[]>(
    savedDeckState?.collectionSets ?? [],
  )
  const [collectionGroups, setCollectionGroups] = useState<string[]>(
    savedDeckState?.collectionGroups ?? [],
  )
  const [collectionMode, setCollectionMode] = useState<CollectionMode>(
    savedDeckState?.collectionSets?.length ? (savedDeckState?.collectionMode ?? 'none') : 'none',
  )
  const [prioritizeDeckHealth, setPrioritizeDeckHealth] = useState(
    savedDeckState?.prioritizeDeckHealth ?? true,
  )
  ...
  const [limitedRecommendations, setLimitedRecommendations] = useState(
    savedDeckState?.limitedRecommendations ?? false,
  )
  ...
  const [activeSubThemes, setActiveSubThemes] = useState<string[]>(
    savedDeckState?.activeSubThemes ?? [],
  )
  const [dismissedSubThemes, setDismissedSubThemes] = useState<string[]>(
    savedDeckState?.dismissedSubThemes ?? [],
  )
  ...
  const [showBuilder, setShowBuilder] = useState(
    () =>
      (usableInitialRoute?.view ?? (savedDeckState?.commander ? 'builder' : 'start')) === 'builder',
  )
  const [activeModal, setActiveModal] = useState<AppModal | null>(
    () => usableInitialRoute?.modal ?? null,
  )
  ...
  const selectedDeckCard = useMemo(
    () =>
      selectedDeckCardLocation
        ? ((selectedDeckCardLocation.board === 'deck' ? deck : sideboard)[
            selectedDeckCardLocation.index
          ] ?? null)
        : selectedCollectionCard,
    [deck, selectedCollectionCard, selectedDeckCardLocation, sideboard],
  )
  const showDeckCard = activeModal === 'card' && selectedDeckCard !== null
  const selectedDeckCardIsCommander =
    selectedDeckCardLocation?.board === 'deck' &&
    selectedDeckCardLocation.index < commanderNames(commander).length
  ...
  const [preferredPrintSet, setPreferredPrintSet] = useState(
    savedDeckState?.preferredPrintSet ?? '',
  )
  ...
  const [deckTargets, setDeckTargets] = useState<DeckTargets>(
    savedDeckState?.deckTargets ?? defaultDeckTargets,
  )
  ...
  const [deckName, setDeckName] = useState(
    () => loadSavedDecks().find(({ id }) => id === savedDeckState?.savedDeckId)?.name ?? '',
  )
  ...
  const currentDeckState = useMemo<PersistedDeckState | null>(
    () =>
  ...
  useEffect(() => {
    const route =
      ...
      ({
        app: appHistoryKey,
        view: savedDeckState?.commander ? 'builder' : 'start',
        modal: null,
        entry: false,
      } satisfies AppHistoryState)
    ...
    const applyRoute = () => {
      const next =
        readAppRoute() ??
        ({ app: appHistoryKey, view: 'start', modal: null, entry: false } satisfies AppHistoryState)
  ...
  useEffect(() => {
    if (
      !commanderDetails ||
      commanderDetails.printings.every((printings) =>
      ...
      .then((printings) => {
            ...
            current && {
              ...
              selections: printings.map((options, index) =>
                Math.max(
                  ...
                  options.findIndex(
                    (printing) =>
                      printing.image === current.images[index] && printing.finish === 'nonfoil',
                  ),
                ),
              ),
            },
        ...
        setDeck((current) =>
          current.map((card, index) => {
            ...
            const matching = printings[index].findIndex(
            ...
            const selectedIndex =
              matching >= 0
                ...
                : Math.max(
                    0,
                    printings[index].findIndex(
                      (printing) => printing.image === card.image && printing.finish === 'nonfoil',
                    ),
                  )
  ...
  useEffect(() => {
    void fetch('https://api.scryfall.com/cards/collection', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ identifiers: basicNames.map((name) => ({ name })) }),
    })
      .then((response) =>
        response.ok ? (response.json() as Promise<{ data: ScryfallCard[] }>) : Promise.reject(),
      )
  ...
  useEffect(() => {
    void fetch('https://api.scryfall.com/sets')
      .then((response) =>
        response.ok ? (response.json() as Promise<{ data: ScryfallSet[] }>) : Promise.reject(),
      )
      .then(({ data }) =>
        setSetOptions(
          data
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const result = response.ok
          ? ((await response.json()) as { data: ScryfallCard[] })
          : { data: [] }
        setCardSearchResults(
          result.data
            .filter(
              (card, index, cards) => cards.findIndex((item) => item.name === card.name) === index,
            )
            .slice(0, 8),
        )
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const result = (await response.json()) as {
          data?: { name: string; mana_cost?: string; card_faces?: { mana_cost?: string }[] }[]
        }
        const cards = result.data ?? []
        setMatches([...new Set(cards.map((card) => card.name))].slice(0, 6))
        setCommanderCosts((current) => ({
          ...current,
          ...Object.fromEntries(
            cards.map((card) => [
              card.name,
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const response = await fetch(
          `https://api.scryfall.com/cards/search?q=${encodeURIComponent(`is:commander id=${identity}`)}&order=edhrec`,
          { signal: controller.signal },
        )
        const result = (await response.json()) as {
          data?: { name: string; mana_cost?: string; card_faces?: { mana_cost?: string }[] }[]
  ...
  useEffect(() => {
    ...
    const missing = shown.filter(
      (name) => commanderCosts[name] === undefined || commanderImages[name] === undefined,
    )
      ...
      for (const name of missing) {
        ...
        for (const cardName of commanderNames(name)) {
          ...
          const card = (await response.json()) as {
            ...
            card_faces?: { mana_cost?: string; image_uris?: { normal: string } }[]
          }
  ...
  async function fallbackRecommendations(identityColours: string[]) {
    ...
    const [mainResponse, manaResponse] = await Promise.all([
      fetch(
        `https://api.scryfall.com/cards/search?q=${encodeURIComponent(`${baseQuery} -t:land -o:"add {"`)}&order=edhrec`,
      ),
      fetch(
        `https://api.scryfall.com/cards/search?q=${encodeURIComponent(`${baseQuery} (t:land or o:"add {")`)}&order=edhrec`,
      ),
    ])
    ...
    const main = allowed(((await mainResponse.json()) as { data: ScryfallCard[] }).data).sort(
      () => Math.random() - 0.5,
    )
    const mana = allowed(((await manaResponse.json()) as { data: ScryfallCard[] }).data).sort(
      () => Math.random() - 0.5,
    )
  ...
  async function fetchCollectionCards(identityColours: string[], selectedSets: string[]) {
    ...
    return cards
      .filter((card) => powerTarget !== 'precon' || !preconFastMana.has(card.name))
      .filter((card, index, all) => all.findIndex((item) => item.name === card.name) === index)
  }

  async function collectionRecommendations(
    identityColours: string[],
    selectedSets = collectionSets,
  ) {
  ...
  async function themeRecommendations(identityColours: string[], themes: string[]) {
    ...
    return cards
      .filter((card) => powerTarget !== 'precon' || !preconFastMana.has(card.name))
      .map((card) => toCard(card, 'Interesting new pick', `theme ${themes.join(' ')}`))
  }
  ...
  async function edhrecRecommendations(slug: string) {
    ...
    for (let index = 0; index < entries.length; index += 75) {
      const cardsResponse = await fetch('https://api.scryfall.com/cards/collection', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const loadPrintings = useCallback(
    async (
      cards: Card[],
      preferredSet = '',
      selectedCollectionSets = collectionSets,
      selectedCollectionMode = collectionMode,
    ) => {
      const analysis = analyseDeck(deck)
      const pickedTags = new Set([
        ...deck.slice(commanderNames(commander).length).flatMap((card) => card.tags),
      ...
      const neededRoles = new Set(
        prioritizeDeckHealth
          ? targetKeys.filter((key) => analysis.counts[key] < deckTargets[key])
          : [],
      )
      ...
      const specialCards = new Set(
        [0, 4].flatMap((start) => {
          ...
          return recommended &&
            score(recommended) >= recommendedScoreThreshold &&
            Math.random() < 0.5
            ? [recommended]
            : []
        }),
      ...
      for (const offered of cards.slice(0, 8)) {
        ...
        const printings = orderedPrintings(
          offered,
          result.data.flatMap((printing) => {
            const image =
        ...
        const selectedIndex =
          special >= 0
            ? special
            : collectionPrinting >= 0
              ? collectionPrinting
              : preferredPrintingIndex(printings, preferredSet)
  ...
  async function start(name: string, preserveDeck = false) {
    ...
    try {
      const responses = await Promise.all(
        commanderNames(chosen).map((name) =>
      ...
      const commanders = await Promise.all(
        responses.map((response) => response.json() as Promise<CommanderCard>),
      )
      ...
      const commanderPrintings = await Promise.all(
      ...
      if (!preserveDeck)
        setDeck(
          commanders.map((card, index) => ({
            ...
            priceUri: card.purchase_uris?.tcgplayer,
            tags: cardTags(card),
            printings: commanderPrintings[index],
            printing: 0,
            finish: commanderPrintings[index][0].finish,
          })),
        )
      ...
      if (activeCollectionMode !== 'only') {
        ...
        try {
          const themeCards = await themeRecommendations(identityColours, [
            theme,
      ...
      if (activeCollectionMode !== 'none' && activeCollectionSets.length) {
        try {
          ...
          if (activeCollectionMode === 'only') offeredCards = collectionCards
            ...
            offeredCards = [
              ...collectionCards,
              ...offeredCards.filter((card) => !collectionNames.has(card.name)),
            ]
          }
        } catch (error) {
      ...
      if (preserveDeck)
        offeredCards = offeredCards.filter(
          (card) =>
            ...
            ![...deck, ...sideboard].some((deckCard) => deckCard.name === card.name),
        )
      ...
      const rankingContext = {
        ...
        activeSubThemes,
        pickedTags: new Set([
          ...rankingDeck.flatMap((card) => card.tags),
          ...Object.entries(preferenceScores)
            .filter(([, score]) => score > 0)
            .map(([tag]) => tag),
        ]),
      ...
      offeredCards = rankRecommendationCards(
        offeredCards,
        rankingContext,
        includeCreature,
        rolesForCard,
  ...
  function addRecommendationCard(card: Card) {
    ...
    if (deck.length < 100)
      setDeck((list) => (list.some((item) => item.name === card.name) ? list : [...list, added]))
    else
      setSideboard((list) =>
        list.some((item) => item.name === card.name) ? list : [...list, added],
      )
  ...
  function decide(card: Card, action: 'add' | 'later' | 'ignore') {
    ...
    if (previous === action) {
      ...
      if (action === 'ignore')
        setIgnoredCards((current) => current.filter((name) => name !== card.name))
    ...
    if (previous !== 'add' && action === 'add') {
      ...
      if (deck.length < 100)
        setDeck((list) =>
          card.typeLine.includes('Basic Land') || !list.some((item) => item.name === card.name)
            ? [...list, added]
            : list,
        )
      else
      ...
      setCommanderDetails(
        (current) =>
          current && {
            ...
            art: current.art.map((image, index) =>
              index === commanderIndex ? (selected.art ?? image) : image,
            ),
            selections: current.selections.map((value, index) =>
  ...
  async function cycleSelectedDeckCardPrinting() {
    ...
    if (selectedDeckCardLocation.board === 'deck')
      await cycleDeckPrinting(selectedDeckCardLocation.index)
  ...
  function deckList(format: ExportFormat) {
    ...
    if (format === 'csv')
      return [
        ...
        ...sideboard.map((card) => `${section([card])},Sideboard`),
      ].join('\n')
    if (format === 'moxfield')
  ...
  const toDeckCard = (card: ScryfallCard): DeckCard => ({
    name: card.name,
    layout: card.layout ?? 'normal',
    typeLine: card.type_line,
  ...
  async function addBasicLands(plan: { name: string; count: number }[]) {
    ...
    try {
      const cards = await Promise.all(
        plan.map(async ({ name, count }) => ({ card: await fetchBasic(name), count })),
      )
      setDeck((current) =>
        [
          ...current,
  ...
  async function selectManualCard(card: ScryfallCard) {
    ...
    const response = await fetch(card.prints_search_uri)
    if (!response.ok) return
    const printings = ((await response.json()) as { data: ScryfallCard[] }).data.filter(
      (printing) => printing.image_uris?.normal ?? printing.card_faces?.[0]?.image_uris?.normal,
    )
  ...
  function handleCardSearchKeys(event: KeyboardEvent<HTMLElement>) {
    ...
    const focusable = [
      ...(cardSearchDialog.current?.querySelectorAll<HTMLElement>(
    ...
    if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
      ...
      const next =
        event.key === 'ArrowDown'
          ? Math.min(index + 1, focusable.length - 1)
          : Math.max(index - 1, 0)
  ...
  async function hydrateDeckCardDetails(card: DeckCard) {
    try {
      ...
      setDeck((current) =>
        current.map((item) => (isSamePrinting(item) ? { ...item, ...metadata } : item)),
      )
      setSideboard((current) =>
        current.map((item) => (isSamePrinting(item) ? { ...item, ...metadata } : item)),
      )
  ...
  function openDeckCard(card: DeckCard, location: DeckCardLocation) {
    ...
    if (
      ...
      void hydrateDeckCardDetails(card)
  }
  ...
  async function applyImportedDeck(imported: ImportedDeck) {
    ...
    if (commanderEntries.reduce((sum, card) => sum + card.quantity, 0) > 2)
    ...
    const identifiers = imported.cards.map((card) =>
    ...
    if (stillMissing.length)
      throw new Error(
        `Not found: ${stillMissing.map(({ name }) => name).join(', ')}. Check spelling, set, and collector number.`,
      )

    const expanded = imported.cards.flatMap((entry, index) =>
    ...
    const illegal = expanded.find(({ card }) =>
      card.color_identity.some((colour) => !identity.includes(colour)),
    )
    ...
    const importedMain = expanded
      ...
      .sort((a, b) => Number(b.entry.board === 'commander') - Number(a.entry.board === 'commander'))
      .map(toImportedCard)
    ...
    setQueue((current) =>
      current.filter(
        (card) => !expanded.some(({ card: importedCard }) => importedCard.name === card.name),
      ),
    )
  ...
  const savedDecksModal = showSavedDecks && (
    <div
      className="modal-backdrop"
      onMouseDown={(event) => {
  ...
  const deckCardModal = showDeckCard && selectedDeckCard && (
    <div
      className="modal-backdrop"
      onMouseDown={(event) => {
    ...
    >
      ...
      >
        <div className="export-heading">
          <p className="eyebrow">{selectedCollectionCard ? 'Collection card' : 'Deck card'}</p>
          <ModalCloseButton
            autoFocus
        ...
        {!selectedDeckCardIsCommander && selectedCollectionCard === null && (
  ...
  const inferredSubThemes =
    activeSubThemes.length < 2
      ? sharedThemes(deckCards, [theme, ...activeSubThemes])
          .filter(
            (name) =>
              deckCards.filter((card) => card.tags.includes(name)).length >
  ...
  const visibleBatch = pairCards.length
    ? [...pairCards, ...rawBatch.filter((card) => !pairCards.includes(card))]
    : rawBatch
  const inferredThemeOptions = [
    ...new Set(deck.slice(commanderNames(commander).length).flatMap((card) => card.tags)),
  ].filter((name) => supportedThemes.includes(name))
  const subThemeOptions = [
    ...new Set([...commanderSubThemes, ...inferredThemeOptions, ...supportedThemes]),
  ]
  ...
  const healthSuggestions =
    recommendationStyle === 'story' && !prioritizeDeckHealth
      ? queue
          .slice(4)
          .filter(
            (card, index, cards) =>
  ...
  const representativeSpellCount = deck
    ...
    .filter((card) => !card.typeLine.includes('Land')).length
  const basicLands =
  ...
  ].map((name) => ({ name, cards: indexedDeck.filter(({ card }) => card.name === name) }))
  ...
  const legalBasicNames = commanderDetails?.colours.length
    ? commanderDetails.colours.map(
        (colour) => basicLandNames[colour as keyof typeof basicLandNames],
      )
    : ['Wastes']
  ...
  const displayedTypeCounts = [
    ...
    ['Other', deck.filter((card) => deckSection(card.typeLine) === 'Other').length],
  ] as const
  ...
  const cardReason = (card: Card) => {
    ...
    const missingRole = rolesForCard(card).find(
      (role) => role !== 'lands' && analysis.counts[role] < deckTargets[role],
    )
    if (missingRole) return targetLabels[missingRole]
    const preference = card.tags
      .filter((tag) => pickedTags.has(tag) && (preferenceScores[tag] ?? 0) > 0)
      .sort((a, b) => (preferenceScores[b] ?? 0) - (preferenceScores[a] ?? 0))[0]
  ...
  const recommendationRoleSupply = Object.fromEntries(
    targetKeys.map((role) => [
      role,
      queue.filter((card) => rolesForCard(card).includes(role)).length,
    ]),
  )
  const scoredBatch = visibleBatch.map((card) => ({
    card,
  ...
  const powerTargetLabel =
    powerTarget === 'precon' ? 'Core' : powerTarget === 'upgraded' ? 'Upgraded' : 'High power'
  ...
  const recommendationSettingsModal = showRecommendationSettings && (
    <div
      className="modal-backdrop"
      onMouseDown={(event) => {
        if (event.target === event.currentTarget) closeModal()
      ...
      <div className="workspace">
        <section className="recommendations">
          ...
          {limitedRecommendations && recommendationState === 'idle' && (
            <p className="limited-mode" role="status">
              {collectionMode === 'only'
                ...
                {healthSuggestions.map((card) => {
                  ...
                  return (
                    <article key={card.name}>
                      <span>
                        {role ? targetLabels[role as keyof typeof targetLabels] : 'Deck support'}
                      </span>
                      <b>{card.name}</b>
                      <button type="button" onClick={() => addRecommendationCard(card)}>
                        Add
                      </button>
                    </article>
                  )
