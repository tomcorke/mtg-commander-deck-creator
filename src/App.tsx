...
function FinishedCardImage({ image, alt, finish, effectsEnabled, hasSynergyConnector = false, className = '' }: { image: string; alt: string; finish?: CardFinish; effectsEnabled: boolean; hasSynergyConnector?: boolean; className?: string }) {
  ...
  return <span className={`finished-card ${loadedImage === image ? 'image-ready' : ''} ${effectsEnabled ? 'tilting-card' : ''} ${finishClass}`} style={effectsEnabled ? { '--foil-hue': `${foilHue}deg` } as CSSProperties : undefined} onPointerMove={effectsEnabled ? moveFoil : undefined} onPointerLeave={effectsEnabled ? resetFoil : undefined} onPointerCancel={effectsEnabled ? resetFoil : undefined}><img className={className} src={image} alt={alt} onLoad={() => setLoadedImage(image)} onError={() => setLoadedImage(image)} />{hasSynergyConnector && <span className="synergy-connector" aria-hidden="true" />}{effectsEnabled && finish && finish !== 'nonfoil' && <img className={`finish-edges ${finish}-edges ${className}`} src={image} alt="" aria-hidden="true" />}</span>
}
