    ...
    <section className="export-modal recommendation-settings-modal" role="dialog" aria-modal="true" aria-labelledby="recommendation-settings-title" onKeyDown={(event) => { if (event.key === 'Escape') { event.preventDefault(); closeModal() } }}>
      ...
      <div className="recommendation-options recommendation-settings-form">
        ...
        <fieldset className="collection-picker"><legend>Collection affinity</legend>
          ...
          <div className="collection-set-controls"><label className="collection-set-search"><span>Search all sets</span><span className="collection-set-input"><input className="clearable-input" value={collectionSearch} onChange={(event) => setCollectionSearch(event.target.value)} placeholder="Search by set name or code…" aria-label="Search all sets" /><button type="button" className="clear-deck-name" onClick={() => setCollectionSearch('')} aria-label="Clear set search">×</button></span></label>
            <div className="collection-selection"><span>Selected sets</span>{collectionSets.length > 0 ? <div className="collection-chips">{collectionSets.map((code) => <button type="button" key={code} onClick={() => toggleCollectionSet(code)}>{collectionSetLabel(code)} ×</button>)}</div> : <p className="collection-empty">No sets selected yet.</p>}</div>
            ...
            <div className="collection-affinity-actions"><label>Match <select value={collectionMode} disabled={!collectionSets.length} onChange={(event) => chooseCollectionMode(event.target.value as CollectionMode)}><option value="none">No collection preference</option><option value="prefer">Prefer selected collection</option><option value="only">Only selected collection</option></select></label><button type="button" disabled={!collectionSets.length || collectionBrowserState === 'loading'} onClick={() => { closeModal(); void browseCollection() }}>{collectionBrowserState === 'loading' ? 'Loading collection…' : 'Browse collection'}</button></div></div>
