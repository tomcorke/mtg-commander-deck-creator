    ...
    <section className="export-modal recommendation-settings-modal" role="dialog" aria-modal="true" aria-labelledby="recommendation-settings-title" onKeyDown={(event) => { if (event.key === 'Escape') { event.preventDefault(); closeModal() } }}>
      ...
      <div className="recommendation-options recommendation-settings-form">
        ...
        <fieldset className="collection-picker"><legend>Collection affinity</legend>
          ...
          <div className={`collection-set-controls ${collectionSets.length ? 'has-selection' : ''}`}><label className="collection-set-search"><span>Search all sets</span><span className="collection-set-input"><input className="clearable-input" value={collectionSearch} onChange={(event) => setCollectionSearch(event.target.value)} placeholder="Search by set name or code…" aria-label="Search all sets" /><button type="button" className="clear-deck-name" onClick={() => setCollectionSearch('')} aria-label="Clear set search">×</button></span></label>
